#include <iostream>
#include <thread>
#include <vector>
#include <string>
#include <cstring>
#include <winsock2.h>
#include <ws2tcpip.h>
#include <mswsock.h>

#pragma comment(lib, "ws2_32.lib")

class WinsockInit {
public:
    WinsockInit() {
        WSADATA wsaData;
        int result = WSAStartup(MAKEWORD(2, 2), &wsaData);
        if (result != 0) {
            throw std::runtime_error("WSAStartup failed: " + std::to_string(result));
        }
    }
    
    ~WinsockInit() {
        WSACleanup();
    }
};

class TunnelServer {
private:
    SOCKET listen_socket;
    std::string target_host;
    int target_port;
    int listen_port;
    bool is_ipv6_listen;
    bool is_ipv6_target;
    WinsockInit winsock;

public:
    TunnelServer(const std::string& listen_addr, int listen_port, 
                 const std::string& target_host, int target_port)
        : target_host(target_host), target_port(target_port), 
          listen_port(listen_port), listen_socket(INVALID_SOCKET) {
        
        // Determine if listen address is IPv6
        is_ipv6_listen = (listen_addr.find(':') != std::string::npos);
        
        // Determine if target is IPv6
        is_ipv6_target = (target_host.find(':') != std::string::npos);
        
        createListenSocket(listen_addr);
    }
    
    ~TunnelServer() {
        if (listen_socket != INVALID_SOCKET) {
            closesocket(listen_socket);
        }
    }

private:
    void createListenSocket(const std::string& listen_addr) {
        if (is_ipv6_listen) {
            listen_socket = socket(AF_INET6, SOCK_STREAM, IPPROTO_TCP);
            if (listen_socket == INVALID_SOCKET) {
                throw std::runtime_error("Failed to create IPv6 socket: " + std::to_string(WSAGetLastError()));
            }
            
            // Allow IPv4 connections on IPv6 socket
            DWORD no = 0;
            setsockopt(listen_socket, IPPROTO_IPV6, IPV6_V6ONLY, (char*)&no, sizeof(no));
            
            struct sockaddr_in6 addr;
            memset(&addr, 0, sizeof(addr));
            addr.sin6_family = AF_INET6;
            addr.sin6_port = htons(listen_port);
            
            if (inet_pton(AF_INET6, listen_addr.c_str(), &addr.sin6_addr) <= 0) {
                throw std::runtime_error("Invalid IPv6 address");
            }
            
            BOOL reuse = TRUE;
            setsockopt(listen_socket, SOL_SOCKET, SO_REUSEADDR, (char*)&reuse, sizeof(reuse));
            
            if (bind(listen_socket, (struct sockaddr*)&addr, sizeof(addr)) == SOCKET_ERROR) {
                throw std::runtime_error("Failed to bind IPv6 socket: " + std::to_string(WSAGetLastError()));
            }
        } else {
            listen_socket = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
            if (listen_socket == INVALID_SOCKET) {
                throw std::runtime_error("Failed to create IPv4 socket: " + std::to_string(WSAGetLastError()));
            }
            
            struct sockaddr_in addr;
            memset(&addr, 0, sizeof(addr));
            addr.sin_family = AF_INET;
            addr.sin_port = htons(listen_port);
            
            if (inet_pton(AF_INET, listen_addr.c_str(), &addr.sin_addr) <= 0) {
                throw std::runtime_error("Invalid IPv4 address");
            }
            
            BOOL reuse = TRUE;
            setsockopt(listen_socket, SOL_SOCKET, SO_REUSEADDR, (char*)&reuse, sizeof(reuse));
            
            if (bind(listen_socket, (struct sockaddr*)&addr, sizeof(addr)) == SOCKET_ERROR) {
                throw std::runtime_error("Failed to bind IPv4 socket: " + std::to_string(WSAGetLastError()));
            }
        }
        
        if (listen(listen_socket, 10) == SOCKET_ERROR) {
            throw std::runtime_error("Failed to listen on socket: " + std::to_string(WSAGetLastError()));
        }
    }
    
    SOCKET connectToTarget() {
        SOCKET target_socket;
        
        // Use getaddrinfo for better hostname resolution (supports both IPv4 and IPv6)
        struct addrinfo hints, *result = nullptr;
        memset(&hints, 0, sizeof(hints));
        hints.ai_family = AF_UNSPEC; // Allow both IPv4 and IPv6
        hints.ai_socktype = SOCK_STREAM;
        hints.ai_protocol = IPPROTO_TCP;
        
        std::string port_str = std::to_string(target_port);
        int status = getaddrinfo(target_host.c_str(), port_str.c_str(), &hints, &result);
        if (status != 0) {
            std::cerr << "getaddrinfo failed for " << target_host << ": " << gai_strerror(status) << std::endl;
            return INVALID_SOCKET;
        }
        
        target_socket = INVALID_SOCKET;
        
        // Try each address until we find one that works
        for (struct addrinfo* ptr = result; ptr != nullptr; ptr = ptr->ai_next) {
            target_socket = socket(ptr->ai_family, ptr->ai_socktype, ptr->ai_protocol);
            if (target_socket == INVALID_SOCKET) {
                continue;
            }
            
            if (connect(target_socket, ptr->ai_addr, (int)ptr->ai_addrlen) != SOCKET_ERROR) {
                break; // Success
            }
            
            closesocket(target_socket);
            target_socket = INVALID_SOCKET;
        }
        
        freeaddrinfo(result);
        return target_socket;
    }
    
    void handleConnection(SOCKET client_socket) {
        SOCKET target_socket = connectToTarget();
        if (target_socket == INVALID_SOCKET) {
            std::cerr << "Failed to connect to target" << std::endl;
            closesocket(client_socket);
            return;
        }
        
        std::cout << "Tunnel established" << std::endl;
        
        // Create threads for bidirectional data transfer
        std::thread t1([this, client_socket, target_socket]() {
            this->transferData(client_socket, target_socket, "Client->Target");
        });
        
        std::thread t2([this, client_socket, target_socket]() {
            this->transferData(target_socket, client_socket, "Target->Client");
        });
        
        t1.join();
        t2.join();
        
        closesocket(client_socket);
        closesocket(target_socket);
        std::cout << "Tunnel closed" << std::endl;
    }
    
    void transferData(SOCKET from_socket, SOCKET to_socket, const std::string& direction) {
        char buffer[4096];
        int bytes_read, bytes_written;
        
        while (true) {
            bytes_read = recv(from_socket, buffer, sizeof(buffer), 0);
            if (bytes_read <= 0) {
                if (bytes_read == SOCKET_ERROR) {
                    std::cerr << "Error reading from " << direction << ": " 
                              << WSAGetLastError() << std::endl;
                }
                break;
            }
            
            bytes_written = 0;
            while (bytes_written < bytes_read) {
                int result = send(to_socket, buffer + bytes_written, 
                                bytes_read - bytes_written, 0);
                if (result <= 0) {
                    if (result == SOCKET_ERROR) {
                        std::cerr << "Error writing to " << direction << ": " 
                                  << WSAGetLastError() << std::endl;
                    }
                    return;
                }
                bytes_written += result;
            }
        }
    }

public:
    void start() {
        std::cout << "Tunnel server started on port " << listen_port << std::endl;
        std::cout << "Tunneling to " << target_host << ":" << target_port << std::endl;
        
        while (true) {
            struct sockaddr_storage client_addr;
            int client_len = sizeof(client_addr);
            
            SOCKET client_socket = accept(listen_socket, (struct sockaddr*)&client_addr, &client_len);
            if (client_socket == INVALID_SOCKET) {
                std::cerr << "Accept failed: " << WSAGetLastError() << std::endl;
                continue;
            }
            
            // Handle each connection in a separate thread
            std::thread([this, client_socket]() {
                this->handleConnection(client_socket);
            }).detach();
        }
    }
};

class TunnelClient {
private:
    std::string local_host;
    int local_port;
    std::string tunnel_host;
    int tunnel_port;
    bool is_ipv6_local;
    bool is_ipv6_tunnel;
    WinsockInit winsock;

public:
    TunnelClient(const std::string& local_host, int local_port,
                 const std::string& tunnel_host, int tunnel_port)
        : local_host(local_host), local_port(local_port),
          tunnel_host(tunnel_host), tunnel_port(tunnel_port) {
        
        is_ipv6_local = (local_host.find(':') != std::string::npos);
        is_ipv6_tunnel = (tunnel_host.find(':') != std::string::npos);
    }

private:
    SOCKET connectToTunnel() {
        SOCKET tunnel_socket;
        
        // Use getaddrinfo for better hostname resolution (supports both IPv4 and IPv6)
        struct addrinfo hints, *result = nullptr;
        memset(&hints, 0, sizeof(hints));
        hints.ai_family = AF_UNSPEC; // Allow both IPv4 and IPv6
        hints.ai_socktype = SOCK_STREAM;
        hints.ai_protocol = IPPROTO_TCP;
        
        std::string port_str = std::to_string(tunnel_port);
        int status = getaddrinfo(tunnel_host.c_str(), port_str.c_str(), &hints, &result);
        if (status != 0) {
            std::cerr << "getaddrinfo failed for " << tunnel_host << ": " << gai_strerror(status) << std::endl;
            return INVALID_SOCKET;
        }
        
        tunnel_socket = INVALID_SOCKET;
        
        // Try each address until we find one that works
        for (struct addrinfo* ptr = result; ptr != nullptr; ptr = ptr->ai_next) {
            tunnel_socket = socket(ptr->ai_family, ptr->ai_socktype, ptr->ai_protocol);
            if (tunnel_socket == INVALID_SOCKET) {
                std::cerr << "Socket creation failed: " << WSAGetLastError() << std::endl;
                continue;
            }
            
            std::cout << "Attempting to connect to " << tunnel_host << ":" << tunnel_port;
            if (ptr->ai_family == AF_INET6) {
                std::cout << " (IPv6)";
            } else if (ptr->ai_family == AF_INET) {
                std::cout << " (IPv4)";
            }
            std::cout << std::endl;
            
            if (connect(tunnel_socket, ptr->ai_addr, (int)ptr->ai_addrlen) != SOCKET_ERROR) {
                std::cout << "Connected successfully!" << std::endl;
                break; // Success
            }
            
            int error = WSAGetLastError();
            std::cerr << "Connection failed: " << error << std::endl;
            closesocket(tunnel_socket);
            tunnel_socket = INVALID_SOCKET;
        }
        
        freeaddrinfo(result);
        return tunnel_socket;
    }
    
    SOCKET bindLocalSocket() {
        SOCKET local_socket;
        
        if (is_ipv6_local) {
            local_socket = socket(AF_INET6, SOCK_STREAM, IPPROTO_TCP);
            if (local_socket == INVALID_SOCKET) {
                return INVALID_SOCKET;
            }
            
            // Allow IPv4 connections on IPv6 socket
            DWORD no = 0;
            setsockopt(local_socket, IPPROTO_IPV6, IPV6_V6ONLY, (char*)&no, sizeof(no));
            
            struct sockaddr_in6 addr;
            memset(&addr, 0, sizeof(addr));
            addr.sin6_family = AF_INET6;
            addr.sin6_port = htons(local_port);
            
            if (inet_pton(AF_INET6, local_host.c_str(), &addr.sin6_addr) <= 0) {
                closesocket(local_socket);
                return INVALID_SOCKET;
            }
            
            BOOL reuse = TRUE;
            setsockopt(local_socket, SOL_SOCKET, SO_REUSEADDR, (char*)&reuse, sizeof(reuse));
            
            if (bind(local_socket, (struct sockaddr*)&addr, sizeof(addr)) == SOCKET_ERROR) {
                closesocket(local_socket);
                return INVALID_SOCKET;
            }
        } else {
            local_socket = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
            if (local_socket == INVALID_SOCKET) {
                return INVALID_SOCKET;
            }
            
            struct sockaddr_in addr;
            memset(&addr, 0, sizeof(addr));
            addr.sin_family = AF_INET;
            addr.sin_port = htons(local_port);
            
            if (inet_pton(AF_INET, local_host.c_str(), &addr.sin_addr) <= 0) {
                closesocket(local_socket);
                return INVALID_SOCKET;
            }
            
            BOOL reuse = TRUE;
            setsockopt(local_socket, SOL_SOCKET, SO_REUSEADDR, (char*)&reuse, sizeof(reuse));
            
            if (bind(local_socket, (struct sockaddr*)&addr, sizeof(addr)) == SOCKET_ERROR) {
                closesocket(local_socket);
                return INVALID_SOCKET;
            }
        }
        
        if (listen(local_socket, 10) == SOCKET_ERROR) {
            closesocket(local_socket);
            return INVALID_SOCKET;
        }
        
        return local_socket;
    }
    
    void handleConnection(SOCKET local_client_socket) {
        SOCKET tunnel_socket = connectToTunnel();
        if (tunnel_socket == INVALID_SOCKET) {
            std::cerr << "Failed to connect to tunnel server" << std::endl;
            closesocket(local_client_socket);
            return;
        }
        
        std::cout << "Tunnel connection established" << std::endl;
        
        // Create threads for bidirectional data transfer
        std::thread t1([this, local_client_socket, tunnel_socket]() {
            this->transferData(local_client_socket, tunnel_socket, "Local->Tunnel");
        });
        
        std::thread t2([this, local_client_socket, tunnel_socket]() {
            this->transferData(tunnel_socket, local_client_socket, "Tunnel->Local");
        });
        
        t1.join();
        t2.join();
        
        closesocket(local_client_socket);
        closesocket(tunnel_socket);
        std::cout << "Tunnel connection closed" << std::endl;
    }
    
    void transferData(SOCKET from_socket, SOCKET to_socket, const std::string& direction) {
        char buffer[4096];
        int bytes_read, bytes_written;
        
        while (true) {
            bytes_read = recv(from_socket, buffer, sizeof(buffer), 0);
            if (bytes_read <= 0) {
                if (bytes_read == SOCKET_ERROR) {
                    std::cerr << "Error reading from " << direction << ": " 
                              << WSAGetLastError() << std::endl;
                }
                break;
            }
            
            bytes_written = 0;
            while (bytes_written < bytes_read) {
                int result = send(to_socket, buffer + bytes_written, 
                                bytes_read - bytes_written, 0);
                if (result <= 0) {
                    if (result == SOCKET_ERROR) {
                        std::cerr << "Error writing to " << direction << ": " 
                                  << WSAGetLastError() << std::endl;
                    }
                    return;
                }
                bytes_written += result;
            }
        }
    }

public:
    void start() {
        SOCKET local_socket = bindLocalSocket();
        if (local_socket == INVALID_SOCKET) {
            throw std::runtime_error("Failed to create local listening socket");
        }
        
        std::cout << "Tunnel client started on " << local_host << ":" << local_port << std::endl;
        std::cout << "Connecting through tunnel at " << tunnel_host << ":" << tunnel_port << std::endl;
        
        while (true) {
            struct sockaddr_storage client_addr;
            int client_len = sizeof(client_addr);
            
            SOCKET client_socket = accept(local_socket, (struct sockaddr*)&client_addr, &client_len);
            if (client_socket == INVALID_SOCKET) {
                std::cerr << "Accept failed: " << WSAGetLastError() << std::endl;
                continue;
            }
            
            // Handle each connection in a separate thread
            std::thread([this, client_socket]() {
                this->handleConnection(client_socket);
            }).detach();
        }
        
        closesocket(local_socket);
    }
};

void printUsage(const char* program_name) {
    std::cout << "Usage:" << std::endl;
    std::cout << "  Server mode: " << program_name << " server <listen_addr> <listen_port> <target_host> <target_port>" << std::endl;
    std::cout << "  Client mode: " << program_name << " client <local_addr> <local_port> <tunnel_host> <tunnel_port>" << std::endl;
    std::cout << std::endl;
    std::cout << "Examples:" << std::endl;
    std::cout << "  IPv4 to IPv6 tunnel server:" << std::endl;
    std::cout << "    " << program_name << " server 0.0.0.0 8080 2001:db8::1 80" << std::endl;
    std::cout << "  IPv6 to IPv4 tunnel server:" << std::endl;
    std::cout << "    " << program_name << " server :: 8080 192.168.1.100 80" << std::endl;
    std::cout << "  Tunnel client:" << std::endl;
    std::cout << "    " << program_name << " client 127.0.0.1 3128 tunnel.example.com 8080" << std::endl;
}

int main(int argc, char* argv[]) {
    if (argc != 6) {
        printUsage(argv[0]);
        return 1;
    }
    
    std::string mode = argv[1];
    
    try {
        if (mode == "server") {
            std::string listen_addr = argv[2];
            int listen_port = std::stoi(argv[3]);
            std::string target_host = argv[4];
            int target_port = std::stoi(argv[5]);
            
            TunnelServer server(listen_addr, listen_port, target_host, target_port);
            server.start();
            
        } else if (mode == "client") {
            std::string local_host = argv[2];
            int local_port = std::stoi(argv[3]);
            std::string tunnel_host = argv[4];
            int tunnel_port = std::stoi(argv[5]);
            
            TunnelClient client(local_host, local_port, tunnel_host, tunnel_port);
            client.start();
            
        } else {
            std::cerr << "Invalid mode. Use 'server' or 'client'." << std::endl;
            printUsage(argv[0]);
            return 1;
        }
        
    } catch (const std::exception& e) {
        std::cerr << "Error: " << e.what() << std::endl;
        return 1;
    }
    
    return 0;
}